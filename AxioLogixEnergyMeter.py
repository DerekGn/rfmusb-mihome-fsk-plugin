...
PARAM_POWER_FACTOR_L = 0x08
PARAM_ACTIVE_POWER_L = 0x09
PARAM_REACTIVE_POWER_L = 0x0A
PARAM_APPARENT_POWER_L = 0x0B
PARAM_CURRENT_N = 0x0C
PARAM_PHASE_ANGLE_N = 0x0D
PARAM_POWER_FACTOR_N = 0x0E
PARAM_ACTIVE_POWER_N = 0x0F
...
def create_energy_meter_sensor(deviceId):
    ...
    create_line_measurements(deviceId, L_LINE_INDEX, 'L')
    create_line_measurements(deviceId, N_LINE_INDEX, 'N')
    create_energy_measurements(deviceId, ENERGY_MEASUREMENTS_INDEX)
...
def update_energy_meter_sensor(deviceId, devices, message, rssi):
    ...
    update_line_measurements(device, L_LINE_INDEX, rssi,
                             Common.find_record(message, PARAM_CURRENT_L),
                             Common.find_record(message, PARAM_PHASE_ANGLE_L),
                             Common.find_record(message, PARAM_POWER_FACTOR_L),
                             Common.find_record(message, PARAM_ACTIVE_POWER_L),
                             Common.find_record(message, PARAM_REACTIVE_POWER_L),
                             Common.find_record(message, PARAM_APPARENT_POWER_L))

    update_line_measurements(device, N_LINE_INDEX, rssi,
                             Common.find_record(message, PARAM_CURRENT_N),
                             Common.find_record(message, PARAM_PHASE_ANGLE_N),
                             Common.find_record(message, PARAM_POWER_FACTOR_N),
                             Common.find_record(message, PARAM_ACTIVE_POWER_N),
                             Common.find_record(message, PARAM_REACTIVE_POWER_N),
                             Common.find_record(message, PARAM_APPARENT_POWER_N))

    update_energy_measurements(device, message, ENERGY_MEASUREMENTS_INDEX, rssi)

def update_line_measurements(device, unitId, rssi, currentRecord, phaseRecord, powerFactorRecord, 
                             activePowerRecord, reactivePowerRecord, apparentPowerRecord):
    # Current
